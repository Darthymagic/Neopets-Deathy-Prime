...
(function () {
  ...
  function parseTrainingTime(blockText, element) {
    if (element) {
      const reduced = element.querySelector('b:not(.strikethrough b)') || element.querySelector('span:not(.strikethrough) b');
      if (reduced) {
        const m = reduced.textContent.match(/(\d+)\s*hrs?,\s*(\d+)\s*minutes?,\s*(\d+)\s*seconds?/i);
        if (m) return Date.now() + (parseInt(m[1])||0)*3600000 + (parseInt(m[2])||0)*60000 + (parseInt(m[3])||0)*1000;
      }
    }
  ...
  function parseVisibleStatusPage() {
    ...
    document.querySelectorAll('td[bgcolor="#efefef"], td[bgcolor="#000000"]').forEach(header => {
      ...
      if (blockText.includes('Course Finished!')) {
        ...
        endTime = parseTrainingTime(blockText, timeTd);
      }
      if (!endTime) return;
